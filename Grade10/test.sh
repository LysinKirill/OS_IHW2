...
run_readers() {
    ...
    for ((i = 1; i <= num_readers; i++)); do
        ./reader >> "TestResults/$test_num/reader_$i.log" &
    done
}
...
run_writers() {
    ...
    for ((i = 1; i <= num_writers; i++)); do
        ./writer >> "TestResults/$test_num/writer_$i.log" &
    done
}
...
mkdir -p "TestResults/$TEST_NUM"

# Initialize shared memory
./init_memory

# Run reader and writer processes
run_readers $num_readers $TEST_NUM
run_writers $num_writers $TEST_NUM
